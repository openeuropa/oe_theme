...
class IconsTextAssert extends BasePatternAssert {
  ...
  protected function assertItems(array $expected_items, Crawler $crawler): void {
    ...
    foreach ($expected_items as $index => $expected_item) {
      ...
      if ($index == 0) {
        self::assertNotContains('ecl-u-mt-l', $list_element->attr('class'));
      }
      else {
        self::assertContains('ecl-u-mt-l', $list_element->attr('class'));
      }
      self::assertEquals($expected_item['text'], $list_element->text());
