  ...
  public function testSiteHeader(): void {
    foreach (['ec', 'eu'] as $component) {
      ...
      if ($component === 'ec') {
        ...
        $this->assertStringContainsString('oe_theme/dist/ec/images/logo/positive/logo-ec--en.svg', $picture->find('css', 'source')
          ->getAttribute('srcset'));
        ...
        $this->assertEquals('https://european-union.europa.eu/index_en', $logo->getAttribute('href'));
        $this->assertEquals('Home - European Union', $logo->getAttribute('aria-label'));
        $picture = $assert->elementExists('css', 'picture.ecl-picture.ecl-site-header__picture', $logo);
        $this->assertEquals('European Union', $picture->getAttribute('title'));
        // Assert desktop logo for EU.
        $this->assertStringContainsString('oe_theme/dist/eu/images/logo/standard-version/positive/logo-eu--en.svg', $picture->find('css', 'source')
          ->getAttribute('srcset'));
        $this->assertEquals('(min-width: 996px)', $picture->find('css', 'source')
          ->getAttribute('media'));
        // Assert mobile logo for EU.
        $this->assertStringContainsString('oe_theme/dist/eu/images/logo/condensed-version/positive/logo-eu--en.svg', $picture->find('css', 'img.ecl-site-header__logo-image')
          ->getAttribute('src'));
        $this->assertEquals('European Union flag', $picture->find('css', 'img.ecl-site-header__logo-image')
          ->getAttribute('alt'));
      }
      ...
      $this->assertEquals('Select your language', $language_container->find('css', 'div.ecl-site-header__language-header div.ecl-site-header__language-title#ecl-site-header__language-title')
        ->getText());
      ...
      $this->assertEquals('true', $close_button->find('css', 'span.ecl-button__container span.ecl-u-sr-only')
        ->getAttribute('data-ecl-label'));
      $this->assertEquals('Close', $close_button->find('css', 'span.ecl-button__container span.ecl-u-sr-only')
        ->getText());
      ...
      $this->assertEquals('<use xlink:href="/build/themes/custom/oe_theme/dist/' . $component . '/images/icons/sprites/icons.svg#close-filled" xmlns:xlink="http://www.w3.org/1999/xlink"></use>', $icon->getHtml());
      ...
      $this->assertEquals('Search', $search_form->find('css', "div.ecl-form-group.ecl-form-group--text-input label[for='edit-keys'].ecl-form-label.ecl-search-form__label")
        ->getText());
      ...
      $this->assertStringContainsString('Menu', $menu_container->find('css', "a.ecl-link.ecl-link--standalone.ecl-menu__open[href][data-ecl-menu-open]")
        ->getText());
      ...
      $this->assertEquals('Leaf item', $first_item->find('css', "a.ecl-menu__link[href='http://leaf.eu'][data-ecl-menu-link]")
        ->getText());
      ...
      $this->assertEquals('Child 1', $second_item->find('css', "div.ecl-menu__mega ul li:nth-child(1) a.ecl-menu__sublink[href='http://child-1.eu']")
        ->getText());
      $this->assertEquals('Child 2', $second_item->find('css', "div.ecl-menu__mega ul li:nth-child(2) a.ecl-menu__sublink[href='http://child-2.eu']")
        ->getText());
