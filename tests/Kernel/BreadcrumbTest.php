...
class BreadcrumbTest extends EntityKernelTestBase {
  ...
  public function testBreadcrumbRendering(): void {
    ...
    $actual = $crawler->filter('nav.ecl-breadcrumb-standardised');
    $this->assertCount(1, $actual);

    // Check if the number of rendered list item is correct.
    $li_item_count = $crawler->filter('ol.ecl-breadcrumb-standardised__container li.ecl-breadcrumb-standardised__segment');
    $this->assertCount(3, $li_item_count);

    // Check if the number of rendered links is correct.
    $links_count = $crawler->filter('ol.ecl-breadcrumb-standardised__container li.ecl-breadcrumb-standardised__segment a.ecl-breadcrumb-standardised__link');
    ...
    $current_page = $crawler->filter('ol.ecl-breadcrumb-standardised__container li.ecl-breadcrumb-standardised__current-page');
    ...
    $position = 0;
    foreach ($links as $title => $url) {
      $link = $crawler->filter('ol.ecl-breadcrumb-standardised__container li.ecl-breadcrumb-standardised__segment a.ecl-breadcrumb-standardised__link')->eq($position);
