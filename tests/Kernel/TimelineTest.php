...
class TimelineTest extends AbstractKernelTestBase {
  ...
  public function testTimelineRender(): void {
    ...
    $build = $display->build($node);
    $output = $this->renderRoot($build);
    $this->assertStringNotContainsString('.ecl-timeline2__item--toggle', (string) $output);
    $this->assertStringNotContainsString('.ecl-timeline2__item--collapsed', (string) $output);
  }
